...
public class BoardController {
	...
	@GetMapping("/board")	// board 테이블을 줘 -> selection
	// controller니까 board 테이블의 데이터를를 view에 담아서 줘/ restcontroller면 board 테이블의 데이터를 그냥 줘
	public String getBoard(Model model) {	// view 에 담아서
		// 더미 데이터 만들기
		List<Board> boardList = new ArrayList<>();
		boardList.add(new Board(1, "스프링1강", "컨트롤러 배우기"));	//mvc 패턴
		boardList.add(new Board(2, "스프링2강", "템플릿 엔진 배우기"));
		boardList.add(new Board(3, "스프링3강", "디비 연결 하기"));
		
		model.addAttribute("boards", boardList)	;	// 키값이 boardList
		return "board/list";	 //board폴더에 list.jsp
		
	}
	
	// 한건보기
	@GetMapping("/board/{id}")	// id에 따라 model에 담는 데이터 달라짐
